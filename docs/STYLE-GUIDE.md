...
`ts/doc/` and the four READMEs, the repository's and one per runtime.
The Rust-port series, the feasibility reports and the defect ledgers are
...
a clean run over the gated set: 273 alerts across 8 files. Those numbers
...
`ts/doc/`. The Go and Rust packages document themselves in
`go/README.md` and `rs/README.md` alone, which are gated like the other
READMEs; the web component's `web/README.md` is outside the gated set.
