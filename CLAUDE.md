...
repository: the two decisions that shape the parser, the repository map
(TypeScript, Go, Rust, the web component), the single-sourced embedded-grammar
workflow, the shared spec fixtures and parity contract, build & test
...
**Held back, by the maintainer's instruction (2026-09-23).** `web/`
keeps `typescript` at 6.0.3. TypeScript 7 ships no JavaScript compiler
API, and `dts-bundle-generator`, which bundles `chess-view.d.ts` into a
self-contained declaration, needs it. `ts/` and every other dependency
track the latest release. `renovate.json` holds the same pin. Lift it when
`dts-bundle-generator` supports TypeScript 7, on the maintainer's
instruction.
